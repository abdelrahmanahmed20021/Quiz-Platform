...
function start_app(e:any) {
    ...
    answers.forEach(ele => {
        const correct_answer = document.querySelectorAll(".correct_answer");
        const falsey_answer_p = document.querySelectorAll(".falsey_answer_p");
        ele.addEventListener('click', (e) => {
            chek.main_checker( e.target as HTMLElement);
            quest_index_num.scrollTop = 20;
            console.log(quest_index_num.scrollTop);
            console.log(falsey_answer_p.length);
        })
    })
    //======= Checker || In || Chossen =======>
