...
const src_quesion_img = document.querySelector(".src_quesion_img");
const poper_img = document.querySelector("#poper_img");
const poper_img_head = document.querySelector("#poper_img h1");
const poper_deleter = document.querySelector(".poper_deleter");
const black_btn = document.querySelector(".black-btn");
const poper_appender = document.querySelector(".poper_appender");
const black_wolf = document.querySelector(".black-wolf");
const quest_index_answer = document.querySelector(".quest_index_answer");
const sec_screen = document.querySelector(".sec_screen");
...
class Checker {
    ...
    Repeat(data) {
        if (this.darwer.index == data.length) {
            ...
            description === null || description === void 0 ? void 0 : description.classList.remove("description_hidde");
...
function start_app(e) {
    if (sec_screen.requestFullscreen) {
        ...
        full_screen.innerText = "Close the full page Mode";
    }
    //======= Audio || Controls =======>
    audio.play();
    ...
    answers.forEach(ele => {
        ele.addEventListener('click', (e) => {
            ...
            quest_index_num.scrollTop = -45 * (unRight_answers + right_answers);
        });
    });
    //======= Checker || In || Chossen =======>
    ...
    remove_2.addEventListener("click", (e) => {
        const deleter = new Delet(e.target, chek.darwer.data, chek.darwer.index);
        deleter.swper();
    });
    //======= Remove || Two || Answers =======>
    //===== Simple || Animate ========>
...
//========= Handel  || Hover  || Effect  || in  || Small  || Deveices ||  ============>
poper_deleter === null || poper_deleter === void 0 ? void 0 : poper_deleter.addEventListener("click", () => {
    poper_img.classList.add("hide_poper");
    if (sec_screen.requestFullscreen) {
        sec_screen.requestFullscreen();
    }
});
poper_appender === null || poper_appender === void 0 ? void 0 : poper_appender.addEventListener("click", () => {
    poper_img.classList.remove("hide_poper");
    // if(document.exitFullscreen) {
    //     document.exitFullscreen();
    //   }
});
description === null || description === void 0 ? void 0 : description.addEventListener("click", () => {
...
full_screen.addEventListener("click", () => {
    if (full_screen.innerText == "Close the full page Mode") {
        if (document.exitFullscreen) {
            document.exitFullscreen();
            full_screen.innerText = "Active the full page Mode";
        }
    }
    else {
        if (sec_screen.requestFullscreen) {
